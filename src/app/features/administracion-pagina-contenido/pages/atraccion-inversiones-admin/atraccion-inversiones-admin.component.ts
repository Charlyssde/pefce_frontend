...
@Component({
  selector: 'app-atraccion-inversiones-admin',
  templateUrl: './atraccion-inversiones-admin.component.html',
  styleUrls: ['./atraccion-inversiones-admin.component.css']
})
export class AtraccionInversionesAdminComponent implements OnInit, AfterViewInit {
  ...
  async getContent() {
    await this.haService.getContent('atraccionInversiones').subscribe(async (resp) => {
      if (resp) {
  ...
  async getLanguages(dataLang: any) {
    ...
    if (arrMimes.indexOf(metadata.type) > -1) {
      if (typeof (FileReader) !== 'undefined') {
        ...
        reader.onerror = (e: any) => {
          this.appC.cargando = false;
          this.lib.printSnackbar(15, null, null, "El archivo no corresponde a un formato de imagen compatible (png, jpg, jpeg o gif)", 5, false, null, null);
        };
      }
      else {
        this.appC.cargando = false;
  ...
  async cancelChangeFile(key: string) {
  ...
  async uploadFile(key: string) {
    ...
    let
      ...
      headers = new Headers(),
      strPath = 'portal/atraccionInversiones/imagenes/' + key + '.'+extension;
  ...
  uploadVideo(key: string) {
    ...
    let
      ...
      formData: FormData = new FormData(),
      headers = new Headers(),
      strPath = 'portal/atraccionInversiones/videos/' + key + '.'+extension;
  ...
  async saveData(section: string) {
    ...
    this.haService.updateContent('atraccionInversiones', params).subscribe((resp) => {
      this.appC.cargando = false;
